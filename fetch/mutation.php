...
if(isset($_POST["query"]))
{
 ...
 $query = "
  SELECT HEX(Mutation.id), Mutation.date, Mutation.test, Mutation.comment FROM Mutation, Patient
  WHERE Mutation.id = {$enc_search} AND Mutation.id = Patient.id AND INSTR('".$roles."', Patient.study) > 0
 ";
}
else
{
