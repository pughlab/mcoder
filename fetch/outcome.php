...
if(isset($_POST["query"]))
{
 ...
 $query = "
  SELECT HEX(Outcome.id), Outcome.date, Outcome.status, Outcome.comment FROM Outcome, Patient
  WHERE Outcome.id = {$enc_search} AND Outcome.id = Patient.id AND INSTR('".$roles."', Patient.study) > 0
 ";
}
else
{
 $query = "
  SELECT * FROM Outcome, Patient WHERE Outcome.id LIKE '%ZZZZZZZZZZZZZZ%' AND Outcome.id = Patient.id AND INSTR('".$roles."', Patient.study) > 0 ORDER BY Patient.id
 ";
}
...
<?php
  ...
  echo '<span style="color:#143de4;text-align:center;"><i class="glyphicon glyphicon-info-sign"></i><b> Cancer disease status have been registered for this patient:</b></span>';
 $output .= '
 <br><br>
...
<tr>
 ...
 while($row = mysqli_fetch_array($result))
 {
  ...
  <div id="comment_outcome_<?php echo $nb;?>" class="modal fade" role="dialog">
