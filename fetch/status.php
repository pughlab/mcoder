...
if (isset($_POST["query"])) {
 ...
 $query = "
  ...
  WHERE ClinicalCondition.id = UNHEX(?)
 ...
 $query = "
   SELECT * FROM ClinicalCondition, Patient
    WHERE ClinicalCondition.id LIKE '%ZZZZZZZZZZZZZZ%'
    AND ClinicalCondition.id = Patient.id
...
$('#statusdata tfoot th').each( function () {
    var title = $(this).text();
...
<div id="delete_status_<?php echo $rowNumber; ?>" class="modal fade" role="dialog">
 ...
 echo $output;
  ...
  <body>
  <?php
 echo '<span style="color:#349A0A;text-align:center;"><i class="glyphicon glyphicon-ok"></i><b> No clinical evaluation has been registered yet for this patient.</b></span>';
}
