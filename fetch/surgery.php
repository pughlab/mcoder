...
if (isset($_POST["query"])) {
 ...
 $query = "
  ...
  WHERE Surgery.id = {$enc_search}
  AND FIND_IN_SET(Patient.study, '".$roles."') > 0
 ";
} else {
 $query = "
  SELECT * FROM Surgery, Patient WHERE Surgery.id LIKE '%ZZZZZZZZZZZZZZ%' AND Surgery.id = Patient.id AND INSTR('".$roles."', Patient.study) > 0 ORDER BY Patient.id
 ";
}
...
$('#surgerydata tfoot th').each( function () {
    var title = $(this).text();
    $(this).html( '<input type="text" placeholder="'+title+'" />' );
} );

          var table = $('#surgerydata').DataTable({
            dom: 'Bfrtip',
            buttons: [
              'copy', {
                extend: 'csv',
                filename: '<?php echo $search; ?>_surgery',
                exportOptions: {
    ...
    <span style="color:#143de4;text-align:center;">
      ...
      <strong>Surgery procedures have been registered for this patient:</strong>
    </span>
    <br><br>
    <table id="surgerydata" class="row-border hover order-column" style="width:100%">
      <thead>
        <tr>
...
<?php
 ...
 while ($row = mysqli_fetch_array($result)) {
  ...
  <tr>
   ...
   <td>'.$row[5].'</td>
   <td>'.$row[6].'</td>
   <td align="center"><a href="#" role="button" class="btn btn-info" data-toggle="modal" data-target="#comment_surgery_'.$rowNumber.'" > <i class="glyphicon glyphicon-zoom-in"></i> </a></td>
   <input type="hidden" name="rowComments' . $rowNumber . '" value="' . $row[6]. '" />';
   if ($hasAdminRole) {
    $output .= '<td align="center">
        <a href="#" role="button" class="btn btn-danger" id="delete_surgery_'. $rowNumber .'_btn" data-toggle="modal" data-target="#delete_surgery_' . $rowNumber . '">
  ...
  <div id="comment_surgery_<?php echo $rowNumber;?>" class="modal fade" role="dialog">
...
<?php if ($hasAdminRole) {?>
<div id="delete_surgery_<?php echo $rowNumber; ?>" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Delete surgery procedure</h4>
      </div>
      <div class="modal-body">
      ...
      <div class="modal-footer">
        ...
        <button
          ...
          onclick="deleteSurgery(document.getElementById('delete_surgery_<?php echo $rowNumber; ?>_btn'))">
            Delete
        </button>
      </div>
    </div>
 ...
 echo $output;
  ...
  <body>
  <?php
 echo '<span style="color:#349A0A;text-align:center;"><i class="glyphicon glyphicon-ok"></i><b> No surgery procedures have been registered yet for this patient.</b></span>';
}
