...
(function(io, $, undefined) {
	...
	io.add = function(opts) {
		$('#load').click(function(e) {
			//io.load(e, opts);
			var id = document.getElementById("patientidsource").value;
			console.log("id: " + id);
			io.loads(opts);
			//console.log("opts");
		});
	...
	io.save = function(opts){
		var dataset = JSON.stringify(pedcache.current(opts));
		//io.save_file(opts, dataset);
		console.log(opts);
		var id = document.getElementById("patientidsource").value.replace( /\s+/g, '');
				...
				$.ajax({
					...
					success:function(data){
						if(data=="Success") {
							...
							setTimeout(function() {
									swal({
											title: "Family pedigree saved!",
											text: "You can browse the database records to visualise the newly added data!\nThe ped file download will start after closing this window. Your tracking ID is: " + tracking,
											type: "success",
    ...
    ("0" + m.getUTCSeconds()).slice(-2);
		...
		$.ajax({    //create an ajax request to display.php
			...
			success: function(response){
				...
				if (response != "None"){
				location.replace("tree.php?dataset="+response+"&id="+id+"&ip="+ip+"&datesystem="+datesystem+"&email="+email+"&username="+username+"&roles="+roles);
			}
			else {
				swal("Error!", "No family pedigree has been recorded for this patient!", "error");
		...
		//var dataset = document.getElementById("responsecontainer").value;
		//console.log("dataset: " + dataset);

	//	location.replace("tree.php?dataset=" + dataset);
	};
