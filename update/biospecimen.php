<?php
	...
	$oldDate = $oldData["collectionDate"];
	$oldType = $oldData["specimenType"];
	$oldStorage = $oldData["storage"];
	$oldBankingID = $oldData["bankingID"];
	try {
		...
		WHERE 
			...
			AND `date` = '$oldDate'
			AND `type` = '$oldType'
			AND `storage` = '$oldStorage'
			AND `bankingid` = '$oldBankingID'";
